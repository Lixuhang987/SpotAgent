...
- 待收尾：图片附件已落 Blob/Stub 但尚不能被 LLM 直接理解；SessionWindow 当前用户气泡尚不展示附件；workspace / permission 文件缓存仍需失效策略；tool 设置 UI 与热加载尚未完成；权限气泡、工作区 UI 与 SessionWindow 自动重连还需要端到端手工验证。
