...
`apps/thread-window-web` 是 WKWebView 承载的 React ThreadWindow 前端。本文只记录修改本目录代码前必须知道的包内前提；整体调用链和 apps 层职责见上级文档。
...
- Swift 只负责加载 Web bundle、注入配置和初始 prompt；平台 tool 走独立 `/api/platform`。
