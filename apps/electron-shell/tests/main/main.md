...
- 改 `agent_server.health`、`thread_window.prepared`、`thread_window.prepare_failed`、`thread_window.closed`、ActivityWindow native focus 兜底或 ActivityWindow prompt fallback 语义时，优先在 `electronShellRuntime.test.ts` 加断言。
