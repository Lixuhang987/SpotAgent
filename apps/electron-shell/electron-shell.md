...
- visible ThreadWindow 关闭后，Electron main 会销毁当前 ActivityWindow `BrowserWindow` 并重新创建一个 `showInactive()` 的小窗，用新的 native window identity 避免旧 ActivityWindow 保持 AXMain / native focused 状态而吞掉下一次同 App 内点击。
