...
- `ElectronShellRuntime` 不直接 import Electron API；它只依赖 `prewarmer`、`activityWindow`、`send`、`stopSupervisor`、`quit` 这组接口，便于测试 command ack、health gate 和预热重入。
...
- ActivityWindow 点击无法聚焦 ThreadWindow 时，main 只发送 `prompt_panel.show_requested` 给 Swift，不自己创建 PromptPanel。
