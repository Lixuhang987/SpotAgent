...
- visible ThreadWindow 关闭后，runtime 会调用 ActivityWindow `blur()` 释放 native focus / AXMain 状态，让下一次点击重新产生 focus 或 mouse event；这不能创建 ActivityWindow，也不能触发 PromptPanel。
