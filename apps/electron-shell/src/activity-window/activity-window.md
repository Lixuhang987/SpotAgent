...
- `focusThread` 最终由 main 校验 sender 后处理。没有 active thread 或无法聚焦时，由 main 请求 Swift 打开 PromptPanel。packaged macOS 下如果点击只触发 ActivityWindow native focus 或 `before-mouse-event` 而未送达 renderer IPC，main 侧 native 兜底使用同一套聚焦 / PromptPanel 语义。
