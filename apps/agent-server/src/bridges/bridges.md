...
权限桥的绑定由 `server/attachThreadSocketHandlers` 在 `op.submit` 时建立；`thread.resume` 会在 snapshot 发出后为恢复中的 running thread 建立绑定并触发 pending request replay。如果 core 发起 permission ask 时没有 thread id 或没有当前绑定连接，直接返回 deny。
