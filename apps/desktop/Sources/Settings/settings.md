...
| 文件 | 职责 |
|------|------|
| `SettingsView.swift` | 设置容器，挂"模型" / "工具" / "Plugin" / "追加" / "MCP" / "权限" / "快捷键" / "工作区"八个 Tab，统一 warm-canvas 背景 |
...
- **视觉风格**：设置页面使用 `SettingsTabBar`、`SettingsFieldStyle`、`SettingsRow`、`SettingsListSection` 等共享样式，与 PromptPanel / StatusBubble 等 SwiftUI 原生界面保持统一 `DESIGN.md` warm-canvas / coral 视觉。不要使用系统 `Form` / `GroupBox` / `.grouped` 样式。窗口标题栏设为透明 + fullSizeContentView；React ThreadWindow 的视觉风格由 `apps/thread-window-web` 独立约束。
