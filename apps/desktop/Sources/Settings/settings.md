...
- **视觉风格**：设置页面使用 `SettingsTabBar`、`SettingsFieldStyle`、`SettingsRow`、`SettingsListSection` 等共享样式，与 PromptPanel / StatusBubble 等 SwiftUI 原生界面保持统一 `DESIGN.md` warm-canvas / coral 视觉。不要使用系统 `Form` / `GroupBox` / `.grouped` 样式。窗口标题栏设为透明 + fullSizeContentView，并固定 `NSAppearance(.aqua)`，避免系统暗色模式把 segmented picker / field 文本渲染成浅色；React ThreadWindow 的视觉风格由 `apps/thread-window-web` 独立约束。
