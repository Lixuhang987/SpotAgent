...
- **视觉风格**：设置页面使用 `settingsCard()` 卡片容器 + `SettingsFieldStyle` 输入框 + `SettingsRow` 行布局，与 PromptPanel / StatusBubble 等 SwiftUI 原生界面保持统一暗色玻璃风格。不要使用系统 `Form` / `GroupBox` / `.grouped` 样式。窗口标题栏设为透明 + fullSizeContentView；React ThreadWindow 的视觉风格由 `apps/thread-window-web` 独立约束。
