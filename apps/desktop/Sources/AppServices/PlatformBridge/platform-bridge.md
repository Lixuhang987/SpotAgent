...
`PlatformBridgeService` 是桌面 App 进程内的独立平台请求处理器。它不维护自己的 socket；`AppServerClient` 在共享 `AppServerConnection` 连接成功后发送 `channel: "platform"` 的 `platform_bridge_hello`，并把同一连接上收到的 `platform_request` 分派给 `PlatformBridgeService`。service 调用 `MacPlatformProvider` 后编码 `platform_response`，仍通过共享连接回到 `agent-server`，让 server 通过 `RemotePlatformAdapter` 调用 macOS 原生能力（剪贴板 / 前台 App / 窗口列表等）。

设计关键：

- **共享连接，独立语义**：platform RPC 与 thread / turn 主语义使用同一条 `AppServerConnection`，但通过 `channel: "platform"` 分派，不占用 `threadId` 路由。
...
- **重连归属**：断线与自动重连由 `AppServerConnection` 统一负责；重连成功后 `AppServerClient` 会重新发送 `platform_bridge_hello`。
...
sequenceDiagram
  ...
  Client->>Server: shared WebSocket send
...
2. 确认 desktop 到 `ws://127.0.0.1:4317/api/thread` 只有一条 WebSocket；该连接同时承载 `ThreadCommand` / `ClientResponse` / `PlatformBridgeMessage`。
