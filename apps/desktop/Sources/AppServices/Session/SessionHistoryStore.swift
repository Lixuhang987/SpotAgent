...
struct SessionHistoryMessage: Identifiable, Equatable {
    let id: String
    let role: String
    let text: String
}

struct SessionHistoryDetail: Equatable {
...
final class SessionHistoryStore {
    ...
    init(directory: URL = SessionHistoryStore.defaultDirectory) {
    ...
    func load(sessionID: String) -> SessionHistoryDetail? {
        guard let record = loadRecord(from: path(for: sessionID)) else { return nil }
    ...
    private func loadRecord(from url: URL) -> SessionHistoryRecord? {
        guard
        ...
        return SessionHistoryRecord(metadata: metadata, messages: messages)
    }

    private func makeEntry(record: SessionHistoryRecord) -> SessionHistoryEntry {
    ...
    private func parseMessages(from messages: [[String: Any]]) -> [SessionHistoryMessage] {
...
private struct SessionHistoryRecord {
