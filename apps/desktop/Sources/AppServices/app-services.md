...
| `AppServicesProductionImpls.swift` | 生产实现：`ProductionHotkeyRegistrar`（委托 Hotkey 层绑定 `KeyboardShortcuts.Name`）+ `ProductionSessionWindowPresenter` / `ProductionSettingsWindowPresenter`（构建 `NSWindow` + `NSHostingController` + 关闭通知监听）+ `ProductionFatalAlertPresenter` |
