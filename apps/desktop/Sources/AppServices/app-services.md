...
| 目录 | 文档 | 职责 |
|------|------|------|
| `AgentServer/` | [agent-server.md](/Users/mu9/proj/handAgent/apps/desktop/Sources/AppServices/AgentServer/agent-server.md) | `AppServer` 统一内核：启动 node 子进程、维护 `/api/platform` WebSocket、默认路径订阅 `/api/activity` 轻量状态、协调平台桥接与健康状态 |
