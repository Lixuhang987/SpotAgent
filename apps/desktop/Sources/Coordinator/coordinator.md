...
- PromptPanel 提交语义：默认路径复用全局 Swift WKWebView ThreadWindow；Electron flag 路径发送 `thread_window.open_initial_prompt` 给 Electron main。两条路径都只传 initial prompt payload，React 收到后通过 `/api/thread` 发送 `thread.start`，再在 `thread.started` 后发送首轮 `input.submit` 和 attachments。ThreadWindow 底部 composer 在已有 active tab 中继续提交 `input.submit`；运行中输入会先进入 React 本地 FIFO，等 thread 离开 running 后逐条发送。
