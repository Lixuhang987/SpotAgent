...
| `SessionWindowLifecycle.swift` | 持有全局唯一 SessionWindow；提供 `createTabWithInitialPrompt / openOrFocusHistory / focus / close`，并把 tab 状态同步到 `SessionRegistry` |
...
- Action prompt 由 PromptPanel 先渲染 template。skill action 只携带渲染后的 prompt 创建新 session；plugin action 额外携带 `{ pluginId, promptName }` 作为 `actionBinding` 创建新 session。
...
- 持有 `SessionWindowLifecycle`、`SettingsLifecycle`，分别闭环会话窗口与设置窗口生命周期。
