...
- PromptPanel 显示后由 Coordinator 隐藏预热全局 [ThreadWindow](/Users/mu9/proj/handAgent/apps/desktop/Sources/ThreadWindow/thread-window.md) 的 `WKWebView`；提交 prompt 后才显示该窗口并创建新的 thread tab。当前 active tab 不会接收 PromptPanel 的初始提交。
