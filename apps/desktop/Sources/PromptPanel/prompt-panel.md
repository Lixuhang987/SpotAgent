...
| `PromptPanelViewModel.swift` | `@Observable` 状态：`draft` / `focusSeed` / `filteredActions` / `selectedActionId` / `attachments` / `submissionDisabledMessage`；支持 `updateActions(_:)` 刷新 action、过滤结果默认首项选中、上下键切换选中 action、Tab 提交选中 action；`onSubmit` / `onSubmitAction` / `onHide` / `onOpenSettings` / `onPreviewImage` 回调出口 |
...
| `PromptPanelStyles.swift` | `PromptPanelContainerModifier` / `ActionRowModifier` / icon button hit area / trigger pill，以及 AppKit `NSScrollView` 透明轨道 + token 化 thumb 的 PromptPanel 统一滚动条样式 |
...
PromptPanel.show() → 记录当前前台应用 → `orderFrontRegardless` 展示非激活面板 → 下一轮 runloop 触发 `focusSeed`
...
PromptPanelGrowingTextView command → PromptPanelInputCommand.resolve
                               ...
                               ├─ Return → ViewModel.submit，保留 PromptPanel 原始提交语义
                               ├─ Tab → ViewModel.submitSelectedAction
...
3. 当前过滤结果非空时默认选中第一项；上下键只在当前过滤结果中移动 `selectedActionId`，并循环切换。
4. 选中 action 时按 Tab 会直接提交该 action；如果当前 draft 已是同一 trigger 的参数形式，则复用 draft 参数，否则无参数 action 直接渲染，有必填参数 action 先预填 `trigger [arg: ]` 并显示缺参提示。
5. Return 始终只走 PromptPanel 自身 `submit()`：普通 prompt 直接提交；如果用户明确手写了 trigger / 参数，仍按既有 ActionInvocation 语义提交。
6. Desktop 本地渲染 `template`。skill action 只提交渲染后的 prompt；plugin action 同时发送 `{ pluginId, promptName }` 作为 `actionBinding`。
7. Coordinator 通过 `thread.start` 强制创建新 thread；Action prompt 不会写入右侧当前展示的既有 thread。
8. agent-server 重新读取同一 plugin manifest 校验绑定，并把 manifest 中的 `mcpServerIds` 持久化到 thread metadata。
...
- **测试不展示真实面板**：`PromptPanelPresentationMode.visible` 是生产默认值；`AppServices.testing()` 和直接测试可使用 `.hiddenForTesting`，仍创建 `NSPanel` / `NSHostingView` 并触发 `focusSeed`，但不执行 `orderFrontRegardless()`、`makeKey()` 或 ESC local monitor，避免 `swift test` 时弹出 PromptPanel 干扰桌面。
...
- **输入框高度与键盘语义**：PromptPanel 输入使用 `PromptPanelGrowingTextView` 包装 AppKit `NSTextView + NSScrollView`。输入框随文本自动增高，最多显示 5 行；超过 5 行后固定高度并出现垂直滚动条。普通 Return 始终提交当前 draft；Shift/Option + Return 插入换行；上下键切换 action 选中；过滤结果存在时默认首项高亮；只有 Tab 提交当前选中 action。
- **滚动条样式统一**：PromptPanel 内由 AppKit `NSScrollView` 承载的滚动区域统一走 `PromptPanelScrollbarStyle`。约束是：轨道和 corner 必须透明，不得露出系统白色 gutter；thumb 颜色从当前 `AppTheme` 的 `textPrimary` 派生半透明态与 hover 态，视觉上继承 PromptPanel 原背景，而不是引入独立浅色条带。
