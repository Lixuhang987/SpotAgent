...
| 文件 | 职责 |
|------|------|
| `SessionWindowView.swift` | 纯 UI：历史侧栏、状态条、消息列表、错误 banner、权限审批气泡、输入框，全部消费 Theme token |
| `SessionViewModel.swift` | `@Observable` 状态：`messages` / `status` / `error` / `pendingPermissionRequests` / `historyList`；消费 `SessionEvent` 维护 UI 状态 |
| `SessionSocketClient.swift` | `URLSessionWebSocketTask` 包装：连接、收发 `SessionMessage`、解析 `SessionEvent`，并发送历史读写和权限响应帧 |
