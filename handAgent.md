...
Electron UI shell Phase 3 只在 `HANDAGENT_ELECTRON_SHELL=1` 时启用。该路径由 Swift 启动 Electron，Electron 监督 agent-server，在 agent-server ready 后主动预热隐藏 ThreadWindow，并在 PromptPanel submit、openHistory 和 focus 时展示或聚焦 Electron `BrowserWindow` ThreadWindow；同时由 Electron 承载 React StatusBubble，renderer 直接订阅 `/api/activity`。Swift 不发送 `thread_window.prepare`，PromptPanel show/toggle 不触发 Electron 预热。默认路径仍保持 Swift `AppServer` 启动 agent-server、Swift `WKWebView` 承载 ThreadWindow，并显示 Swift StatusBubble。平台能力仍只通过 Swift `/api/platform` 执行。

### 分层职责

- `apps/desktop`：负责宿主生命周期、热键、PromptPanel、Settings、焦点恢复、默认路径 Swift StatusBubble，以及通过 `MacPlatformProvider` 实现 macOS 原生能力（ScreenCaptureKit / NSWorkspace / NSPasteboard 等）。默认路径仍管理全局唯一 Swift `NSWindow/WKWebView` ThreadWindow，并只订阅 `/api/activity` 的轻量 activity 状态更新 Swift `ThreadRegistry`；Electron flag 路径通过 ThreadWindow / ActivityWindow command bridge 路由，Swift 不 mirror Electron activity 状态。
...
- Swift desktop 不持有 thread client，不发送 `ThreadCommand`，不解析 `ThreadNotification`；默认路径只订阅 `/api/activity` 的 `AgentActivityEvent` 更新 Swift `ThreadRegistry`，供 Swift StatusBubble 展示和点击回跳。默认路径还负责 Swift `NSWindow/WKWebView` host 和 initial prompt 注入；Electron flag 路径通过 command bridge 把 ThreadWindow host 与 ActivityWindow host 交给 `apps/electron-shell`，Swift StatusBubble 默认关闭且 Swift 不 mirror Electron activity 状态。
