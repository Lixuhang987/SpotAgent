...
- [ ] **2.2 选区未传入 WebSocket**（文本选区已接通；图片附件仅为字符串占位，LLM 看不到真实图像字节）
  - 现状：文本选区已通过 `user_message.attachments` 传到 agent-server 并拼入 prompt；但 `MessageTranslator.composeUserContent` 把 image 附件压成 `[图片附件: image/png (id)]` 字符串占位，LLM 实际看不到图像内容。
