...
- **发现日期**：2026-06-09
- **复现步骤**：
  1. 在 `main` 合入 `366a706 fix: add Electron StatusBubble native focus fallback` 后执行 `bash ./scripts/package-app.sh --mock-llm`。
  ...
  1. 通过全局快捷键打开 PromptPanel，提交 `ELECTRON_STATUSBUBBLE_NATIVE_FOCUS_QA_20260609 [mock:assistant-ok]`。
  1. 等待 Electron `HandAgent ThreadWindow` 和 `HandAgent Activity` 出现，确认 thread 文件 `~/.spotAgent/threads/thread-1780950395783-sxe1nw.json` 包含 user prompt 与 mock assistant。
...
- **实际结果**：点击后 `HandAgentDesktop` 仍没有可见 `PromptPanel`，Electron 仍只有 `HandAgent Activity`。ActivityWindow 在该状态下为 `AXMain=true` / `AXFocused=false`。截图为 `/tmp/handagent-qa/electron-statusbubble-native-focus-after-click.png`。
...
- **证据**：packaged app 资源已包含 `focusable: true`、`acceptFirstMouse: true`、`onNativeFocus?.()` 和 `runtime.handleActivityWindowNativeFocus()`；`/api/activity` snapshot 返回 `activeThreadId: "thread-1780950395783-sxe1nw"`、`status: "idle"`、`latestSummary: "点击开始"`；thread 文件包含 `ELECTRON_STATUSBUBBLE_NATIVE_FOCUS_QA_20260609 [mock:assistant-ok]` 与 `Mock assistant response: main chain is reachable.`；关闭 ThreadWindow 后 `lsof` 显示 node 仍监听 `127.0.0.1:4317`；退出 QA app 后无 HandAgent / Electron / agent-server 残留，`127.0.0.1:4317` 无监听。
- **初步调用链 / 根因边界**：已验证 `PromptPanel submit -> Electron ThreadWindow -> agent-server -> /api/activity activeThreadId -> ActivityWindow 可见 -> packaged app 包含 native focus fallback -> 从其他 App 点击触发 Swift PromptPanel`。失败边界进一步收敛为：ActivityWindow 已经 `AXMain=true` 时，同 App 内后续 CGEvent 点击既没有可靠 renderer click / IPC，也不会触发 native `focus` 事件；仍需继续用 `$trace-and-verify-call-chain` 定位并补足该状态下的点击 fallback。
