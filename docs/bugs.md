...
- **严重级别**：P2
- **发现日期**：2026-06-09
- **当前状态**：代码修复 `42860fe` 已合入主仓库，待默认 WKWebView packaged mock app 实机回归确认后再从当前 bug 清单移除。
- **复现步骤**：
...
- **初步调用链 / 根因边界**：已验证 `agent-server thread.list -> thread.listed -> ThreadWindow 历史侧栏显示 workspace 分组`；自动化定位到 `toggleWorkspaceExpanded` 直接通过 Immer draft 读取/修改 `Set` 会触发 `[Immer] The plugin for 'MapSet' has not been loaded into Immer`。修复改为基于当前状态创建新 `Set` 并补充 `localStorage` 持久化，仍需主仓库 packaged live 回归验证 UI 展开、收起、刷新保持与历史项激活。
