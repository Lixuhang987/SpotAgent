...
- **初步调用链 / 根因边界**：`SessionScopedToolRegistry.refreshActivated()` 激活后仍把 `use_tools` 暴露给 provider，允许模型在已激活 session 中重复调用 no-op meta-tool；这与 `docs/manual-qa.md` 场景 3 “同一 session 激活后不再重复出现 use_tools，模型直接调用真实工具”的验收目标冲突。失败边界位于 session-scoped tool registry 的激活后工具表，而不是 desktop 渲染、权限回灌或持久化层。
