...
- 场景 7 已启动但受同一 assistant 文本截断缺陷阻塞：默认 WKWebView packaged mock app 提交 `THREADWINDOW_SCENE7_LAYOUT_QA_20260609 [mock:assistant-ok]`，生成 `~/.spotAgent/threads/thread-1780956663996-o7g1aj.json`。thread 文件中的 assistant content 是 `Mock assistant response: main chain is reachable.`，但 ThreadWindow 截图 `/tmp/handagent-qa/threadwindow-scenario7-layout-assistant-ok.png` 和裁剪 `/tmp/handagent-qa/threadwindow-scenario7-assistant-final-crop.png` 只显示 `Mock`。该证据已补充到 `docs/bugs.md` 的 “ThreadWindow 最终 assistant 文本只显示首段” 缺陷，并已同步给子 agent `019ea947-5f2d-7982-b734-77dcf5ce7f63`。本轮不把场景 7 判为通过；退出后无 HandAgent / agent-server 残留，`127.0.0.1:4317` 无监听。
