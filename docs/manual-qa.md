...
- 场景 4 已完成部分 live 证据：`~/.spotAgent/workspaces.json` registry 原序为 `default -> tmp -> qa-workspace -> handagent-test`，未修改 workspace 配置；新增 QA fixture thread `qa-scene4-default-workspace.json`、`qa-scene4-tmp-workspace.json`、`qa-scene4-qa-workspace.json`、`qa-scene4-handagent-workspace.json`，分别挂到四个 workspace。默认 WKWebView packaged mock app 提交 `THREADWINDOW_SCENE4_SIDEBAR_QA_20260609 [mock:assistant-ok]` 后生成 `~/.spotAgent/threads/thread-1780952709285-om3grm.json`，截图 `/tmp/handagent-qa/threadwindow-scenario4-initial-sidebar.png` 显示“新建对话”按钮、搜索框、workspace 分组排序为 `default -> handagent-test -> qa-workspace -> tmp -> 默认对话`。AX 识别 sidebar 中存在 `search text field`，输入关键词后过滤默认对话历史项，截图 `/tmp/handagent-qa/threadwindow-scenario4-search-focused-scene4.png`；通过搜索框内 `cancel` 按钮清空后完整列表恢复，截图 `/tmp/handagent-qa/threadwindow-scenario4-search-cleared.png`。AXPress “新建对话”按钮后创建第二个 tab，并显示空 thread 的“等待输入”，截图 `/tmp/handagent-qa/threadwindow-scenario4-new-conversation.png`。场景 4 继续验证时提交 `THREADWINDOW_SCENE4_REMAINING_QA_20260609 [mock:assistant-ok]` 生成 `~/.spotAgent/threads/thread-1780954032424-13pw6s.json`；手动 `/api/thread thread.list` 返回 53 个 thread，其中包含四个 `qa-scene4-*` workspace fixture，但点击 workspace 分组标题、对 heading trigger 执行 AXPress、聚焦 trigger 后按 Space 均无法展开分组，`default` trigger 的 `AXExpanded=false` 且 region 子项数为 0，截图 `/tmp/handagent-qa/threadwindow-scenario4-default-expanded-click.png`、`/tmp/handagent-qa/threadwindow-scenario4-default-expanded-axcoords.png`、`/tmp/handagent-qa/threadwindow-scenario4-default-expanded-keyboard.png`。结论：场景 4 因 workspace 分组标题无法展开而阻塞，缺陷已写入 `docs/bugs.md`；展开状态刷新持久化、workspace fixture 历史项过滤匹配和历史项激活需待修复后继续验证。退出后无 HandAgent / agent-server 残留，`127.0.0.1:4317` 无监听。
