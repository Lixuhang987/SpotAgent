...
- 场景 0 真实 LLM 工具激活验证已发现阻塞缺陷：重新执行 `bash ./scripts/package-app.sh` 后确认 bundle 无 `HandAgentRuntimeMode.json`，即 settings / real LLM 模式；提交 `HANDAGENT_REAL_TOOL_SCENE0_A_20260609 请看一下我的屏幕，并简要说明你看到了什么。` 后，ThreadWindow 显示 `use_tools` 和 `screen.capture` tool result，network 日志 `~/.spotAgent/log/2026-06-09/network-001.jsonl` 证明第一轮请求 tools 仅 `use_tools`、第二轮激活后包含完整工具集、第三轮包含 `screen_capture` function call/output 且 provider 返回 HTTP 200 streaming。但 `/api/activity` 持续为 `waiting / 等待权限确认`，ThreadWindow 无可见权限请求面板，超过 60 秒仍不恢复；`/api/thread` snapshot 和 `~/.spotAgent/threads/thread-1780961051324-1d663h.json` 只有 user message。该问题已写入 `docs/bugs.md`，场景 0 保留待修复后继续验收。证据截图：`/tmp/handagent-qa/real-tool-scene0-permission-waiting-no-panel.png`。
