...
生成脚本必须纳入开发启动流程，不能只作为人工命令存在。所有会启动桌面 App 或 ThreadWindow 前端的本地入口，都必须在启动前先执行 token 生成检查：

- `bash ./scripts/swiftw run HandAgentDesktop` 在构建 Swift 与 Electron/React 前先运行 token 生成脚本。
- `pnpm --filter handagent-thread-window-web dev` 的启动链路先运行 token 生成脚本，再启动 Vite dev server。
- token 源未变化时，生成脚本必须保持幂等，不应造成无意义文件改动。
...
ThreadWindow 的 dev/build 脚本必须依赖 token 生成脚本。开发者启动 Vite 或构建 Web bundle 时，不需要记住额外手工步骤。
...
- token 生成脚本已纳入桌面 App 与 ThreadWindow 前端启动流程，启动前自动刷新生成物。
