...
当前 `screen.capture` 已迁移到 `apps/desktop/Sources/AppServices/PlatformBridge/MacPlatformProvider.swift`，使用 `SCScreenshotManager` 实现 display / window / region 三种 target。旧的 `packages/platform-macos/` 已删除。
