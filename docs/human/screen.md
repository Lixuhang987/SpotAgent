...
这更偏“远程共享 / 协作”场景，不是 agent 本地 OCR 的刚需，但如果你以后做“让别人看某个 ThreadWindow”，这比自己拼分享逻辑强得多。
...
当前 `screen.capture` 已迁移到 `apps/desktop/Sources/AppServices/PlatformBridge/MacPlatformProvider.swift`，使用 `SCScreenshotManager` 实现 screen / display / window / region target。旧的 `packages/platform-macos/` 已删除。

仍未覆盖的能力：

- 流式捕获（`SCStream`）
- 系统 picker（`SCContentSharingPicker`）
- 音频 / 麦克风 / 录制
- HDR
