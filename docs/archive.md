...
### Electron UI Shell StatusBubble tool / waiting / error 状态

- **验证日期**：2026-06-09
- **验证环境**：Electron flag packaged app，`mock-llm`；标准 `open dist/HandAgentDesktop.app` 启动。
- **验证过程**：通过当前 Electron command socket 发送 `thread_window.open_initial_prompt`，分别触发 clipboard tool、permission request、workspace request 和 mock LLM error；同时订阅 `/api/activity`、用 Computer Use / 截图检查 ThreadWindow 与右下 Electron StatusBubble。
- **证据**：`ELECTRON_STATUS_TOOL_CURRENT_QA_20260609 [mock:clipboard-read]` 的 activity 序列包含 `tool_running` / `正在使用 clipboard.read`，thread `thread-1780967402708-r1l1e5` 持久化 `clipboard.read` tool result `HANDAGENT_STATUS_TOOL_QA_20260609_CLIPBOARD`。`ELECTRON_STATUS_PERMISSION_CURRENT_QA_20260609 [mock:permission-write]` 进入 `waitingRequest:"permission"`，截图 `/tmp/handagent-qa/electron-status-waiting-current.png` 显示 StatusBubble `等待确认 / 等待权限确认` 与 ThreadWindow `file.write` 权限面板；拒绝后 thread `thread-1780967417227-j5h13u` 记录 `permission_request file.write deny`。`ELECTRON_STATUS_WORKSPACE_CURRENT_QA_20260609 [mock:workspace-ask]` 允许 `workspace.askUser` tool 权限后进入 `waitingRequest:"workspace"`，截图 `/tmp/handagent-qa/electron-status-workspace-waiting-current.png` 显示 StatusBubble `等待确认 / 等待工作区选择` 与 ThreadWindow workspace 选择面板；选择 `qa-workspace` 后 thread `thread-1780967496598-gbrpwx` 记录 `{"workspaceId":"qa-workspace"}`。`ELECTRON_STATUS_ERROR_CURRENT_QA_20260609 [mock:llm-error]` 进入 `error`，截图 `/tmp/handagent-qa/electron-status-error-current.png` 显示 StatusBubble `出现错误 / 运行失败` 与 ThreadWindow 红色错误气泡 `MockLLMClient forced failure for QA.`，thread `thread-1780967476395-a9hh1l` 持久化同一 error event。
- **结论**：通过。Electron React StatusBubble 能展示 tool running、permission/workspace waiting 与 error 状态；ThreadWindow 对应内联请求面板和错误气泡可见并可交互。

### Electron UI Shell PromptPanel ready gate
