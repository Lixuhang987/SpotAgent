...
| `ModelSettings.ts` | `loadModelSettings()`：同步 `readFileSync` 读 `settings.json` 的 `llm.{model, summarizerModel, apiKey, baseUrl, api}`；JSON 解析失败抛错（带文件路径），其它字段缺失走默认；调用方负责是否加缓存 |
