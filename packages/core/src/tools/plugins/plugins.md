...
本目录实现第一版本地目录插件 tool：从 `~/.spotAgent/plugins/<plugin-id>/plugin.json` 读取 manifest，把每个 tool 包装成 `AgentTool`，执行时通过子进程 JSON stdin/stdout 隔离运行。
...
| `PluginTool.ts` | `AgentTool` 包装器：按 manifest 启动本地可执行文件，向 stdin 写入 `{ input, context, workspace? }`，解析 stdout JSON，处理非 0 exit / invalid JSON / timeout |
...
- `command` 必须是相对插件目录的路径，不能是绝对路径或 `..` 越界路径。
...
插件必须向 stdout 写一个 JSON 值作为 tool 结果。stderr 只在失败时拼入错误信息；非 0 exit、stdout 非 JSON、超时都会作为 tool error 回到 runtime，不会拖垮 agent-server。

## workspace 与权限对齐

- 只有 manifest 声明 `permissions.workspace = "read" | "write"` 时，`PluginTool` 才会解析 `input.workspaceId / input.relativePath`。
- 解析路径复用 builtin 文件 tool 的 workspace 沙箱逻辑：拒绝绝对路径、`..` 越狱，并在 `realpath` 后再次确认仍在 workspace root 内。
- 插件只收到经校验后的 `workspaceRoot / absolutePath`；不会从 LLM 入参直接信任任意主机路径。
