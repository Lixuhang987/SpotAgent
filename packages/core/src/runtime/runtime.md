...
- agent-server 收到 `input.submit` 后先持久化 user message，然后发 `user.message.recorded`；如果该输入唤醒 idle thread，再发 `turn.started`。run 结束后补 `turn.completed` 和 `thread.status.changed`。
