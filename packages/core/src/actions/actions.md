...
| `ActionBinding.ts` | 从 plugin prompt manifest 与 `{ pluginId, promptName }` 解析可持久化的 `SessionActionBinding`；拒绝 `kind: "skill"` 的 prompt 绑定工具 |
...
- `globalShortcut` 是 prompt 级默认 Action 全局快捷键配置，只影响 desktop 宿主，不写入 session metadata。
- `mcpServerIds` 属于 plugin 级别；plugin session 创建后会写入 metadata，后续 runtime 按 session metadata 决定 MCP scope。
