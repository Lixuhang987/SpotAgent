...
标准 MCP client 与 tool adapter。第一版支持 `stdio` 和 `Streamable HTTP`，按 MCP `2025-11-25` 稳定规范实现完整客户端能力：`tools`、`prompts`、`resources` 三类 server-side primitive 全部覆盖。HandAgent thread scope 不依赖 MCP transport session，而是由 thread metadata 的 `actionBinding` 决定。
...
- `mcp.json` 中配置的所有 server 默认作为**全局** MCP server，会被注入每个 thread 的 tool registry，无需依赖 plugin 触发；plugin 的 `mcpServerIds` 与全局列表合并并去重。
...
- stdio server 可配置 `cwd`，用于兼容插件原始配置中的相对 command 路径；也可配置 `requestTimeoutMs`，默认 60s，避免外部 server 卡死时拖挂当前 thread run。
