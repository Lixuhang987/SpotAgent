...
| `FilePermissionPolicy.ts` | 三层策略：`sessionRules`（内存 Map，按 `${sessionId}::${argHash}` 隔离）→ 持久化 `~/.spotAgent/permissions.json`（按 `argHash` 全局命中）→ fallback 调 `askResolver`；`argHash = sha256(toolName + stableJSON(args))` |
