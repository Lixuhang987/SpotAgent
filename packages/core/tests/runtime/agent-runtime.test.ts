...
describe("AgentRuntime", () => {
  it("adds tool-use guidance to LLM requests without persisting it in session messages", async () => {
    ...
    const result = await runtime.runWithMessages([{ role: "user", content: "执行一个流程，使用两个tool调用" }]);

    expect(seenMessages).toHaveLength(1);
    expect(seenMessages[0][0]).toMatchObject({
      role: "system",
      content: expect.stringContaining("structured tool call"),
    });
